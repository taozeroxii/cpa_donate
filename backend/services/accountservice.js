...
module.exports = {
    ...
    onLogin(value){
        return new Promise((resolve,reject) =>{
            // resolve(value)
            connection.query("SELECT du.*,dur.role as default_role_name FROM `donate_user` du LEFT JOIN donate_user_role dur on du.default_role = dur.id where username = ?",[value.username],(error,result)=>{//bind params เพื่อกัน sql injection ด้วย ? [value.username]
                if(error) return reject(error);
                if(result.length > 0){
                    ...
                    if(password_varify(value.password,userLogin.password,userLogin.salz)){// นำค่าที่พิมมาไปเช็คกับรหัสผ่านใน ฐานข้อมูล + salz ว่าตรงกันไหม
                        ...
                        resolve(userLogin);
                    }else {reject(new Error ('Username หรือ Password ไม่ถูกต้อง'))}
