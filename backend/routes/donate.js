    ...
    async (req, res) => {
      try {
        ...
        if(checkinstock[0].remaining_instock ==  "undefined" ){
          res.status(400).error({ message:`ไม่พบรายการรับบริจาคสิ่งของดังกล่าว`});
        }
        else if(checkinstock[0].remaining_instock == null && checkinstock[0].amount_all >=   req.body.amount){
          res.json({ message: await service.insertWDraw(req.body) });
        }
        else if(checkinstock[0].remaining_instock != null && checkinstock[0].remaining_instock >= req.body.amount ){ 
          res.json({ message: await service.insertWDraw(req.body) });
        }
        else { res.status(400).error({ message:`จำนวนสินค้าไม่พอจ่าย คงเหลือทั้งหมด : ${checkinstock[0].remaining_instock} ${checkinstock[0].item_name_type}`}); }
